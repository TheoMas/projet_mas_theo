...
export class AuthRefreshInterceptor implements HttpInterceptor {
  ...
  intercept(req: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
    ...
    const authReq = storedAccess ? req.clone({ setHeaders: { Authorization: `Bearer ${storedAccess}` } }) : req;
          ...
          return this.authService.refreshAccessToken().pipe(
            switchMap((refreshResponse: any) => {
              ...
              if (newAccessToken) {
                ...
                const replay = req.clone({ setHeaders: { Authorization: `Bearer ${newAccessToken}` } });
